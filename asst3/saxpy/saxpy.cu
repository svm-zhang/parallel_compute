...
void saxpyCuda(int N, float alpha, float* xarray, float* yarray, float* resultarray) {
    ...
    double toDeviceEndTime = CycleTimer::currentSeconds();
    double toDeviceDuration = toDeviceEndTime - startTime;
    printf(
        "To Device duration: %.3f ms\t\t[%.3f GB/s]\n",
        1000.f * toDeviceDuration,
        GBPerSec(totalBytes, toDeviceDuration)
    );
    ...
    printf(
        "CUDA kernel Saxpy duration: %.3f ms\n",
    ...
    //
    double toHostStartTime = CycleTimer::currentSeconds();
    cudaMemcpy(
        resultarray, device_result, N*sizeof(float), cudaMemcpyDeviceToHost
    );
    double toHostEndTime = CycleTimer::currentSeconds();
    double toHostDuration = toHostEndTime - toHostStartTime;
    printf(
        "To Host duration: %.3f ms\t\t[%.3f GB/s]\n",
        1000.f * toHostDuration,
        GBPerSec(N*sizeof(float), toHostDuration)
    );
    ...
    // I updated to 4N here.
    printf(
        "Effective BW by CUDA saxpy: %.3f ms\t\t[%.3f GB/s]\n",
        1000.f * overallDuration,
        GBPerSec(totalBytes+N*sizeof(float), overallDuration)
    );
